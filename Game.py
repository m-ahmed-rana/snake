    ...
    def parse_level_file(self):
        ...
        for obj in self.level:
            ...
            if "position" in self.level[obj]:
                obj_position = Vector(self.level[obj]["position"][0] * scale_factor,
                                      self.level[obj]["position"][1] * scale_factor)
            obj_offset = Vector(0, 0)
            if "offset" in self.level[obj]:
                obj_offset = Vector(self.level[obj]["offset"][0] * scale_factor,
                                    self.level[obj]["offset"][1] * scale_factor)
            obj_scale = Vector(0, 0)
            if "scale" in self.level[obj]:
                obj_scale = Vector(self.level[obj]["scale"][0] * scale_factor,
                                   self.level[obj]["scale"][1] * scale_factor)
                obj_scale_int = Vector(int(self.level[obj]["scale"][0] * scale_factor),
            ...
            if obj == "player":
                ...
                if "tile_solids" in self.level[obj]:
                    ...
                    tile_scale = Vector(self.level[obj]["tile_scale"][0] * scale_factor,
                ...
                try:
                    thing = SolidObj(self, obj_position, Vector(int(self.level[obj]["solid"][0] * scale_factor),
...
class PhysicsObj(DrawableObj):
    ...
    def __init__(self, level, position=Vector(0, 0), scale=Vector(0, 0), velocity=Vector(0, 0),
            ...
            if (player.position.y + player.scale.y) < self.rect.top + variable * self.scale_factor:
                ...
                if not self.velocity.x == 0:
                    ...
                    if self.scale_factor == 1.0:
                        ...
                        print
                        "no number specified for this specific scale_factor in the PlatformObj.collision_response()"
...
class Node_dialog(Menu):
    def __init__(self, level, dialog_tree, data_list, position, npc_owner):
        ...
        for choice in self.choice_list:
            ...
            choice_button = Button_dialog(level, scale, button_position, image_deselect, image_select, dialog_tree,
